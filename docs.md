...
## Implementation
**Files:** `index.html` (HTML structure + CDN scripts), `script.js` (core UI: serialization, state, undo/redo, projects panel, preview/graph triggers), `storage.js` (project persistence, IndexedDB image storage, disk/workspace helpers), `boxes/box.js` (base `Box` class + `MathBox`, `TextBox`, `HeaderBox`, `PagebreakBox`; shared box utilities: `renderBoxes`, `focusBox`, `insertBoxAfter`, etc.), `boxes/calcbox.js` (`CalcBox` class + all calc helper functions and display utilities), `boxes/graphbox.js` (`GraphBox` class), `boxes/imagebox.js` (`ImageBox` class), `expr-row.js` (shared expression row builder used by calc and graph), `graph-ui.js` (graph editing mode UI), `preview.js` (preview panel rendering), `math.js` (LaTeX parser, AST evaluator, GLSL/JS/LaTeX codegen, calc evaluator), `graph.js` (GraphRenderer class only), `gpuimage.js` (WebGL framebuffer utility), `gl_utils.js` (WebGL helpers), `style.css` (dark Catppuccin theme).

**Load order:** `gl_util.js` → `gpuimage.js` → `math.js` → `graph.js` → `expr-row.js` → `storage.js` → `boxes/box.js` → `boxes/calcbox.js` → `boxes/imagebox.js` → `boxes/graphbox.js` → `script.js` → `graph-ui.js` → `preview.js`. The box files load before script.js because they define classes used there; graph-ui.js and preview.js load last because they depend on globals defined in script.js.
