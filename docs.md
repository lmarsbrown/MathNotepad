...
- [Box System](#box-system) — data model; math, text, heading, page break, graph, calc, image box types; commented boxes
...
- [Preview Panel](#preview-panel) — paginated MathJax-rendered document view; zoom; print/PDF; blackboard bold shorthands
- [Graph Box & Editor](#graph-box--editor) — graph edit mode; expression list UI; parametric curves; point expressions; snap tooltip; color palette
...
## Implementation
**Files:** `index.html` (HTML structure + CDN scripts), `script.js` (core UI: serialization, state, undo/redo, projects panel, preview/graph triggers), `storage.js` (project persistence, IndexedDB image storage, disk/workspace helpers), `boxes/box.js` (base `Box` class + `MathBox`, `TextBox`, `HeaderBox`, `PagebreakBox`; shared box utilities: `renderBoxes`, `focusBox`, `insertBoxAfter`, etc.), `boxes/calcbox.js` (`CalcBox` class + all calc helper functions and display utilities), `boxes/graphbox.js` (`GraphBox` class), `boxes/imagebox.js` (`ImageBox` class), `graph-ui.js` (graph editing mode UI), `preview.js` (preview panel rendering), `math.js` (LaTeX parser, AST evaluator, GLSL/JS/LaTeX codegen, calc evaluator), `graph.js` (GraphRenderer class only), `gpuimage.js` (WebGL framebuffer utility), `gl_util.js` (WebGL helpers), `style.css` (dark Catppuccin theme).

**Load order:** `gl_util.js` → `gpuimage.js` → `math.js` → `graph.js` → `storage.js` → `boxes/box.js` → `boxes/calcbox.js` → `boxes/imagebox.js` → `boxes/graphbox.js` → `script.js` → `graph-ui.js` → `preview.js`. Box files load before `script.js` because they define classes used there; `graph-ui.js` and `preview.js` load last because they depend on globals defined in `script.js`.
...
- [Calc Box](#calc-box)
- [Image Box](#image-box)
- [Commented Boxes](#commented-boxes)
...
## Implementation
**State:** `let boxes = []` (script.js:7). IDs generated by `genId()` (boxes/box.js).

**Class hierarchy:** All box types are classes extending `Box` (boxes/box.js). `Box` provides the base `createElement()` which builds the `.box` div with drag handle, focus/click handlers, and drag-and-drop. Each subclass calls `super.createElement()` and appends its own content. Factory: `createBoxFromType(type, id)` (boxes/box.js) — returns the correct class instance.

**Creation:** `appendBox(type)` (boxes/box.js) inserts after the focused box or at end. `insertBoxAfter(afterId, type)` (boxes/box.js) inserts at a specific position.

**Deletion:** `deleteBox(id)` (boxes/box.js) removes from `boxes`, removes DOM element, calls `cleanupCalcBox` if needed, then calls `syncToText()`.

**DOM construction:** Each box class has a `createElement()` method that builds its full DOM and returns the element. `renderBoxes()` (boxes/box.js) loops over `boxes` and calls `box.createElement()` for each.

**Reconciliation:** `diffAndApply(newBoxes)` (script.js) reconciles a freshly-parsed box array with the current DOM, reusing existing elements by position. Calc boxes always fully rebuild.

**Focus:** `focusBox(id)` (boxes/box.js), `focusBoxAtEdge(id, edge)` (boxes/box.js). `focusedBoxId` tracks the currently focused box.
...
**MathQuill:** `MQ.MathField(span, opts)` created inside `MathBox.createElement()` (boxes/box.js). Field instance stored in `mqFields` map. `edit` handler calls `syncToText()` (after `suppressBoxSync` check). `enter` handler calls `appendBox(defaultBoxType)`.
...
### Motivation
Allows temporarily disabling a box from preview output without deleting it — analogous to commenting out code. Useful for drafting or hiding intermediate work.

### Subfeature index
None.

### Behaviour
A box can be toggled "commented" via `toggleComment(id)` (boxes/box.js). Commented boxes appear visually dimmed in the editor (`.commented` CSS class) and are completely skipped in the preview renderer — they do not contribute any content. They still serialize to the source and are preserved across edits.

### Implementation
**Data:** `box.commented: bool` (not present when false). Set by `toggleComment(id)` (boxes/box.js).

**Serialization:** Every line of the box's normal serialization is prefixed with `%% `. Example:
```
%% \[x^{2}+y^{2}=1\]
```
On parse, a block of `%% `-prefixed lines is stripped of the prefix and re-parsed as normal LaTeX, then the resulting box has `.commented = true` applied.

**Preview:** `createPreviewElement` returns `null` for commented boxes; the preview render loop skips them.
...
```js
{
  ...
  // same calc-mode flags as CalcBox (physicsBasic, physicsEM, physicsChem, useUnits, useSymbolic, useBaseUnits, sigFigs)
  expressions: [{
    id, latex, color, enabled,
    thickness: 2.0,    // line thickness for curves
    pointSize: 8.0,    // point radius for point expressions (default 8)
    draggable: false,  // whether a named-point expression is user-draggable
    sliderMin: 0,      // t/slider range min (default 0)
    sliderMax: 10,     // t/slider range max (default 10) — also used for parametric tMin/tMax via tMin/tMax fields
    tMin: 0,           // parametric t start (default 0; only emitted when non-default)
    tMax: 1,           // parametric t end (default 1; only emitted when non-default)
  }],
  // runtime-only (not serialized):
  _snapshotDataUrl,   // PNG data URL captured when exiting graph mode; shown in preview thumbnail
  _viewBounds,        // { xMin, xMax, yMin, yMax } — saved camera state, restored on re-entry
}
```
Created by `new GraphBox(id)` (boxes/graphbox.js). Color palette: `GRAPH_COLORS` (script.js), cycled by `nextGraphColor()` (script.js).

**Serialization:**
```
\begin{graph}{W}{H}[{light}][{units}][{symbolic}][{base_units}][{physics}][{physics_em}][{physics_chem}][{sf=N}]
% id color on|off thickness sliderMin sliderMax [ps:N] [drag] [t:min:max] latex
\end{graph}
```
Optional brace tokens on the header line match corresponding box flags (same as calc box). Per-expression line fields: `id` (e.g. `ge1`), `color` (CSS hex), `on|off`, `thickness` (float), `sliderMin`, `sliderMax` (both floats, defaults `0`/`10`), then optional tokens `ps:N` (point size), `drag` (draggable point), `t:min:max` (t-range for parametric), followed by the raw LaTeX. Parsed in `parseFromLatex` (script.js) via `\begin{graph}` block handler.
...
```
%@hl [#ffe066]         ← optional fill color hex
\[LaTeX content\]
```
...
### Implementation
`dragSrcBoxId` (script.js:17) tracks the source. `dragstart`, `dragover`, `drop` listeners are set up inside `Box.createElement()` (boxes/box.js). On drop, the `boxes` array is spliced, then `rebuildBoxList()` (script.js) and `syncToText()` are called.
...
%@hl #ffe066
```
Color is optional; `%@hl` with no color means highlighted with no fill. Old format `%@hl filled #ffe066` is still parsed for backwards compatibility. Parsed in `parseFromLatex` (script.js); `pendingHighlight` captures the prefix and `applyPending()` applies it to the next box.

**Editor CSS:** `.box-is-highlighted` class — dark yellow background `#2a2600`, border `#6b5900`. Set in `Box.createElement()` (boxes/box.js) and synced in `diffAndApply`; also toggled directly on the DOM element by the button/key handler for immediate feedback.
...
- [Blackboard Bold & Math Shorthands](#blackboard-bold--math-shorthands)
...
## Blackboard Bold & Math Shorthands

### Motivation
Let users type compact shorthands in MathQuill (or the source panel) that automatically expand to proper LaTeX in the preview, without cluttering the WYSIWYG editor with long commands.

### Subfeature index
None.

### Behaviour
The following substitutions are applied to all math content (display and inline) before MathJax renders it — **in the preview only**; the source and box editor are untouched.

**Blackboard bold:** Double-letter sequences expand to `\mathbb{X}`:
`AA`→`\mathbb{A}`, `BB`→`\mathbb{B}`, …, `RR`→`\mathbb{R}`, `ZZ`→`\mathbb{Z}` (all 26 letters).

**Operator shorthands** (applied first, before BB expansion):
| Shorthand | LaTeX |
|---|---|
| `<=` | `\leq ` |
| `>=` | `\geq ` |
...
These are also applied inside `$...$` inline math in text boxes.

### Implementation
**`BB_SUBS`** (preview.js:8) — constant object mapping double-letter keys to `\mathbb{...}` values.

**`applyBBSubs(latex)`** (preview.js) — applies operator substitutions first (before HTML escaping would corrupt `<`/`>`), then iterates `BB_SUBS` with `replaceAll`, then calls `matrixSub`. Called from `processTextContent` (inline math in text boxes) and from the display-math render path for math boxes.
...
- [Point Expressions](#point-expressions)
...
- Users can add/edit/delete expressions, each with:
  - **Graphable expressions** (implicit x/y, parametric): color swatch + enable toggle; rendered as curves.
  - **Point expressions** (named literal points): rendered as dots; if draggable, can be moved by clicking and dragging on the canvas.
  - **Non-graphable expressions** (constants, bare, inequalities): numeric result shown (same as CalcBox).
- **Parametric expressions** (e.g. `(cos(t), sin(t))`) show a t-range row (min/max inputs) below the expression field.
...
- "Done Editing" returns to normal mode: captures the current canvas as a PNG (`_snapshotDataUrl`) and restores the box list. The snapshot is shown as the graph thumbnail in both the box editor and the preview until the graph is re-entered.
- The graph's pan/zoom state (`_viewBounds`) is saved on exit and restored when the box is re-entered.
- The graph box thumbnail in the box list shows the expression count (e.g. "3 expressions") via `updateGraphBoxCount()` (graph-ui.js).

## Implementation
**Entry/exit:**
- `enterGraphMode(boxId)` (graph-ui.js) — hides box list, builds settings row + expression list, sets `graphModeBoxId`, restores `box._viewBounds` if present, calls `scheduleGraphRender()`.
- `exitGraphMode()` (graph-ui.js) — captures canvas as PNG into `box._snapshotDataUrl`, saves `box._viewBounds`, calls `_teardownGraphModeUI()`, restores box list, calls `box.createElement()` to rebuild the thumbnail with the new snapshot.
...
**T-range row:** `_createTRangeRow()` (graph-ui.js) builds a hidden `.t-range-row` with min/max number inputs appended below each expression wrapper. `updateTRangeRows(exprBoxes, analysis)` (graph-ui.js) iterates all active expression boxes and shows/hides the t-range row based on whether the expression's ID appears in `analysis.parametrics`. Min/max inputs update `exprBox.element.dataset.tMin/tMax`; `commitGraphEditorToBox` reads them into `expr.tMin/tMax`.
...
### Motivation
Allows graphing curves defined by a parameter `t` — e.g. circles `(cos(t), sin(t))`, spirals, Lissajous figures — which cannot be expressed as implicit `f(x,y)=0`.

### Subfeature index
None.

### Behaviour
A parametric expression is entered as `(xExpr, yExpr)` where both expressions may use `t`. Once classified as parametric, a **t-range row** appears below the input field with min/max number inputs (defaults: 0 to 1). The curve is sampled across the t-range and rendered as a connected polyline on the GPU. The t-range is persisted in the expression's `tMin`/`tMax` fields.

### Implementation
- `compileGraphExpressions` (math.js) classifies `(xExpr, yExpr)` expressions as `kind: 'parametric'` and adds them to `analysis.parametrics` as `{ exprId, xExpr, yExpr }`.
- `buildParametricEvaluators(parametrics, analysis)` (math.js) builds JS evaluators `{ evalX(t), evalY(t) }` for each parametric expression.
- The renderer samples the parametric evaluators across `[tMin, tMax]` and draws polylines rather than using the implicit curve shader.
- `updateTRangeRows(exprBoxes, analysis)` (graph-ui.js) shows/hides the `.t-range-row` beneath each expression based on `analysis.parametrics`.
- `tMin`/`tMax` are serialized as optional `t:min:max` token in the graph expression line (only emitted when non-default).
...
### Motivation
Allows plotting named literal points (e.g. `a = (1, 2)`) as visible dots on the graph, and optionally making them draggable so the user can interactively change their position by dragging them on the canvas.

### Subfeature index
None.

### Behaviour
An expression of the form `name = (x, y)` where both `x` and `y` are numeric literals is classified as a point expression. The point is rendered as a filled dot of configurable size (`pointSize`, default 8). If the expression is marked **draggable** (via a checkbox in the color popup when `isDraggableEligible` is true), the user can click and drag the dot to reposition it; dragging updates the expression LaTeX in real time.

### Implementation
- `classifyExpression` (math.js) detects `name = (literal, literal)` pattern; `analyzeExpressions` produces `analysis.staticPoints: [{ xVal, yVal, exprId, isDraggable, varName }]`.
- `compileGraphExpressions` emits `kind: 'point'` entries with `draggable` flag for the renderer.
- Drag callbacks `_onPointDrag(exprId, x, y)` / `_onPointDragEnd(exprId)` (graph-ui.js) update the MathQuill field latex and call `commitGraphEditorToBox` + `scheduleGraphRender`.
- `pointSize` is stored per-expression and serialized as optional `ps:N` token in the graph expression line (only emitted when non-default 8).
- The `draggable` flag is serialized as optional `drag` token in the graph expression line.
...
- [gl_util (WebGL Utilities)](#gl_util-webgl-utilities)
...
### Behaviour
- **`classifyExpression`** identifies whether an expression is a function definition (`funcdef`), variable definition (`vardef`), parametric (`bareExpr` initially, resolved to `parametric` by `analyzeExpressions`), point expression, or other implicit equation.
- **`analyzeExpressions`** resolves dependencies between definitions, detects cycles in function definitions (DFS), determines which definitions depend on x/y, pre-evaluates constants on the CPU, and detects parametric and point expressions.
- Expression `kind` values: `'funcdef'`, `'vardef'`, `'bareExpr'` (bare equation/inequality — may be reclassified), `'parametric'` (parametric curve `(xExpr, yExpr)`), `'point'` (named literal point `name=(x,y)`).
...
- `_parseAndClassifyCached(latex)` (math.js) — module-level parse+classify cache keyed by LaTeX string; ASTs are immutable so sharing across calls is safe
- `analyzeGraphExpressionsOnly(expressions)` (math.js) — runs parse (cached) through constant evaluation and bareExpr classification, but skips shader/evaluator generation; used for the fast constant-only update path
- **Incremental update logic** (graph.js `updateExpressions`): tracks `_perExprStructKey` and `_perExprKind` per expression ID; when only `definition-const` (pure scalar, no x/y) expressions change, takes fast path: calls `analyzeGraphExpressionsOnly`, updates `_constantValues` and named-point positions in `_compiledExprs`, skips full shader recompile
...
**Preview rendering** (`createPreviewElement`, `type === 'calc'` branch, preview.js):
...
**Display helpers in boxes/calcbox.js:**
...
|---|---|---|
| `ImageBox.createElement()` | boxes/imagebox.js | Builds full box DOM; handles all interactions |
| `cleanupImageBox(boxId)` | script.js | Revokes stored blob object URL; does NOT delete IDB record |
| `initImageDb()` | storage.js | Opens IndexedDB on startup |
| `idbSaveImage(projectId, filename, mimeType, arrayBuffer)` | storage.js | Writes image record to IDB |
| `loadImageAsObjectURL(box)` | storage.js | Tries disk (if workspace connected) then IDB fallback; returns blob URL or null |
| `reconnectWorkspace()` | storage.js | Requests workspace permission with user gesture; sets `currentWorkspaceHandle` |
| `retryAllDiskImages()` | storage.js | Re-renders all file-mode boxes; calls `renderBoxes()` |
| `idbLoadImageAsObjectURL(key)` | storage.js | Reads IDB record, returns a blob object URL |
| `idbGetImageRecord(key)` | storage.js | Returns full record (used by ZIP export) |
| `idbDeleteImage(key)` | storage.js | Deletes one record |
| `idbGetAllImagesForProject(projectId)` | storage.js | Returns all records for a project (used by ZIP export) |
| `idbDeleteAllImagesForProject(projectId)` | storage.js | Bulk-deletes via `by_project` index (called on project delete) |
| `idbMigrateImageKeys(oldId, newId)` | storage.js | Re-keys `draft/` records when a new project is first saved |

**IndexedDB schema:** Database `mathnotepad_imagedb` v2:
- Object store `images`: primary key `key` (`"${projectId}/${filename}"`), index `by_project` on `projectId`. Record: `{ key, projectId, filename, mimeType, data: ArrayBuffer }`.
- Object store `handles` (added in v2): stores `FileSystemDirectoryHandle` objects keyed by project ID, used for workspace persistence across page reloads.
...
**Integration points:**
- `renderBoxes()` (boxes/box.js): calls `box.createElement()` for each box, including `ImageBox`
...
| Function | Location | Purpose |
|---|---|---|
| `loadProjects()` | storage.js | Load from localStorage |
| `saveProjects(list)` | storage.js | Write to localStorage |
| `saveDraftNow()` | storage.js | Immediate draft save |
| `saveDraft()` | storage.js | Debounced draft save |
| `saveUiState()` | storage.js | Save panel widths/visibility |
| `saveCurrentProject()` | storage.js | Save to named project entry |
| `openProject(id)` | storage.js | Load project, parse, rebuild |
| `downloadCurrentProject()` | storage.js | Export as .tex or .zip file |
| `deleteProject(id)` | storage.js | Remove from list |
| `renameProject(id)` | storage.js | Edit project title inline |
| `restoreFromLatex(latex)` | storage.js | Parse latex into boxes, rebuild |
| `markDirty()` / `markClean()` | storage.js | Update `isDirty` + title dot |
| `newProject()` | script.js | Create blank project |
| `renderProjectsList()` | script.js | Render projects panel DOM |
| `openProjectsPanel()` | script.js | Show slide-in panel |
| `closeProjectsPanel()` | script.js | Hide panel |
...
**Instrumented functions:** `Box.createElement`, `rebuildBoxList`, `diffAndApply`, `setFocused`, `clearFocused`, `focusBox`, `insertBoxAfter`, `appendBox`, `syncToText`, `restoreSnapshot`, and the debounced text-sync timer.
